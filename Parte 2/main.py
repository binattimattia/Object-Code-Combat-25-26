...
def main():
    ...
    while player_1.is_alive() and player_2.is_alive():
        ...
        print(f"{player_1.get_name()} attacks {player_2.get_name()} for {player_1_damage} damage.")
        if player_2.is_alive():
            print(f"{player_2.get_name()} has {player_2.get_health()} HP left.")
        else:
            if player_2.get_health() < 0:
                player_2.set_health(0)
            print(f"{player_2.get_name()} has {player_2.get_health()} HP left.")
            print(f"{player_2.get_name()} has been defeated!")
            break

        player_2_damage = player_2.attack(player_1)
        print(f"{player_2.get_name()} attacks {player_1.get_name()} for {player_2_damage} damage.")
        if player_1.is_alive():
            print(f"{player_1.get_name()} has {player_1.get_health()} HP left.")
        else:
            if player_1.get_health() < 0:
                player_1.set_health(0)
            print(f"{player_1.get_name()} has {player_1.get_health()} HP left.")
            print(f"{player_1.get_name()} has been defeated!")
            break
